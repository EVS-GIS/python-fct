...
def PlotCorridorLimit(ax, data, x, left, right, window=1, basis=2):
    ...
    for k, part in enumerate(parts):
        ...
        if xk.size > 0:
            ...
            ax.plot(
                xk,
                (vbwk[:, 0] - baselinek[:, 0]) + (vbwk[:, 1] - baselinek[:, 1]),
                'darkgray',
                linewidth=1.0,
                # linestyle='dotted',
...
def PlotLandCoverProfile2(ax, x, y, basis=0, window=1, direction='upright'):
